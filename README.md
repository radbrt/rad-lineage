...
RadLineage is A meltano utility extension for RadLineage that wraps the `meltano-lineage` command.
...
```shell
poetry run radlineage_extension --help
poetry run radlineage_extension describe --format=yaml
poetry run radlineage_invoker --help # if you have are wrapping another tool
```
